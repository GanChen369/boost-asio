...
{
public:
    client(boost::asio::io_service& io_service, const std::string& server, const std::string& path):resolver_(io_service), socket_(io_service)
    {
        /**
        ...
        resolver_.async_resolve(query,
                boost::bind(&client::handle_resolve, this, boost::asio::placeholders::error, boost::asio::placeholders::iterator));
...
private:
    ...
    void handle_connect(const boost::system::error_code& err)
    {
        if(!err)
        {
            // The connection was successfully. Send the request.
    ...
    void handle_write_request(const boost::system::error_code& err)
    {
        ...
        {
            /**
             * Read the response status line. The response_ streambuf will automatically grow to accommodate the entire line.
