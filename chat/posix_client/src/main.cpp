...
public:
    posix_chat_client(boost::asio::io_service& io_service, tcp::resolver::iterator endpoint_iterator)
        :socket_(io_service),
...
private:
    void handle_connect(const boost::system::error_code& error)
    {
        if(!error)
        {
            // Read ths fixed-length header of the next message from the server
            boost::asio::async_read(socket_,
                    boost::asio::buffer(read_msg_.data(), chat_message::header_length),
                    boost::bind(&posix_chat_client::handle_read_header, this, boost::asio::placeholders::error));

            // Read a line of input entered by the user
            boost::asio::async_read_until(input_,
                    input_buffer_,
                    '\n',
                    boost::bind(&posix_chat_client::handle_read_input, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
        }
    }
    ...
    void handle_read_header(const boost::system::error_code& error)
    {
        if(!error && read_msg_.decode_header())
        {
            ...
            boost::asio::async_read(socket_,
                    ...
                    boost::bind(&posix_chat_client::handle_read_body, this, boost::asio::placeholders::error));
        }
        else
        {
            close();
    ...
    void handle_read_body(const boost::system::error_code& error)
    {
        if(!error)
        {
            ...
            boost::asio::async_write(output_,
                    buffers,
                    boost::bind(&posix_chat_client::handle_write_output, this, boost::asio::placeholders::error));
        }
        else
        {
            close();
    ...
    void handle_read_input(const boost::system::error_code& error, std::size_t length)
    {
        if(!error)
        {
            ...
            boost::asio::async_write(socket_,
                    ...
                    boost::bind(&posix_chat_client::handle_write, this, boost::asio::placeholders::error));
        }
    ...
    void close()
    {
        // cancel all outstanding asynchronous operation
        socket_.close();
        input_.close();
        output_.close();
    }
